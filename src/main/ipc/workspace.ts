...
import type { WorkspaceNode, WorkspaceSnapshot } from '../../renderer/src/types/workspace'

const MAX_FILE_SIZE = 1_000_000 // 1MB

export function registerWorkspaceHandlers(): void {
  ipcMain.handle('workspace:load', async (_event, rootPath: string): Promise<WorkspaceSnapshot> => {
    ...
    async function buildTree(dirPath: string): Promise<WorkspaceNode[]> {
      ...
      for (const entry of sorted) {
        ...
        if (entry.isDirectory()) {
          ...
          nodes.push({ id: nodeID, name: entry.name, isDirectory: true, children })
        } else {
          nodes.push({ id: nodeID, name: entry.name, isDirectory: false, children: [] })
        }
      }
  ...
  ipcMain.handle('workspace:read-file', async (_event, filePath: string): Promise<string> => {
  ...
  ipcMain.handle('workspace:list-policies', async (_event, rootPath: string) => {
    const policiesDir = join(rootPath, 'policies')
    try {
      const entries = await readdir(policiesDir, { withFileTypes: true })
      return entries
        .filter((e) => !e.isDirectory() && (e.name.endsWith('.yaml') || e.name.endsWith('.yml')))
        .map((e) => ({
  ...
  ipcMain.handle('workspace:read-policy', async (_event, policyPath: string): Promise<string> => {
  ...
  ipcMain.handle(
    ...
    async (_event, rootPath: string, fileName: string, content: string): Promise<string> => {
      const policiesDir = join(rootPath, 'policies')
  ...
  ipcMain.handle('workspace:delete-policy', async (_event, policyPath: string): Promise<void> => {
    const { unlink } = await import('fs/promises')
    await unlink(policyPath)
  })

  ipcMain.handle('workspace:file-extension', (_event, filePath: string): string => {
