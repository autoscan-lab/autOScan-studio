...
import type { WorkspaceNode, FileTab, WorkspaceSnapshot } from '../types/workspace'
...
interface AppState {
  ...
  loadWorkspace: (rootPath: string, restoredExpanded?: string[], restoredSelected?: string | null) => Promise<void>
...
function getOpenFileTabs(nodes: WorkspaceNode[], openIDs: string[]): FileTab[] {
  ...
  nodes.forEach(walk)
  return openIDs.map((id) => ({ id, title: nameMap[id] || id.split('/').pop() || 'unknown' }))
}
...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  loadWorkspace: async (rootPath, restoredExpanded, restoredSelected) => {
    ...
    const storedActivePolicyID = await window.api.storeGet('activePolicyID') as string | null
    const policies = get().policies
    if (storedActivePolicyID && policies.some((p) => p.id === storedActivePolicyID)) {
  ...
  closeTab: (nodeID) => {
    set((s) => {
      ...
      return {
        openFileNodeIDs: ids,
        selectedFileNodeID: needsNewSelection ? (ids[ids.length - 1] ?? null) : s.selectedFileNodeID
      }
    })
  ...
  removeTestCase: (id) => {
    const draft = get().selectedPolicyDraft
    if (!draft) return
    const updated = { ...draft, testCases: draft.testCases.filter((tc) => tc.id !== id) }
    get().updatePolicyDraft(updated)
  ...
  updateTestCase: (id, updates) => {
    ...
    const updated = {
      ...draft,
      testCases: draft.testCases.map((tc) => (tc.id === id ? { ...tc, ...updates } : tc))
    }
    get().updatePolicyDraft(updated)
  },
  ...
  handleEngineEvent: (event) => {
    switch (event.type) {
      case 'started':
        set({ runStatusMessage: `Grading ${event.totalSubmissions} submissions…` })
        break
      case 'discovery_complete':
        set((s) => ({
          runOutputText: s.runOutputText + `Discovered ${event.submissionCount} submissions\n`
        }))
        break
      case 'compile_complete':
        set((s) => ({
          runOutputText: s.runOutputText + `Compiled ${event.results.length} submissions\n`
        }))
        break
      case 'scan_complete':
        set((s) => ({
          runOutputText: s.runOutputText + `Scanned ${event.results.length} submissions\n`
        }))
        break
      case 'run_complete':
        set({
          latestRunReport: event.report,
          runStatusMessage: `Done — ${event.report.summary.totalSubmissions} submissions graded`,
  ...
  restoreSession: async () => {
    const workspacePath = (await window.api.storeGet('workspacePath')) as string | null
    const expanded = (await window.api.storeGet('expandedDirectoryIDs')) as string[] | null
    const selected = (await window.api.storeGet('selectedFileNodeID')) as string | null
    const sidebarMode = (await window.api.storeGet('sidebarMode')) as SidebarMode | null
    const isSidebarVisible = (await window.api.storeGet('isSidebarVisible')) as boolean | null
    const isInspectorVisible = (await window.api.storeGet('isInspectorVisible')) as boolean | null
    const isOutputVisible = (await window.api.storeGet('isOutputVisible')) as boolean | null
  ...
  persist: () => {
    ...
    window.api.storeSet('expandedDirectoryIDs', Array.from(s.expandedDirectoryIDs))
