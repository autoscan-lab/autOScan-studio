  ...
  return (
    <div className="h-full overflow-y-auto p-4 space-y-4">
      ...
      >
        <div className="flex flex-wrap gap-1.5 mb-3">
          {draft.testCases.map((tc) => (
            <button
              ...
              className={`
                px-2.5 py-1 rounded-md text-[11px] font-medium cursor-default
                ${
          ...
          <button
            onClick={addTestCase}
            className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[11px] text-text-primary hover:bg-hover bg-hover cursor-default"
          >
            <MdAdd size={14} />
            Add
          </button>
        ...
        {selectedTC && (
          <div className="space-y-4 rounded-lg border border-separator/50 bg-canvas/50 p-4">
...
function CollapsibleSection({
  ...
  return (
    <section className="rounded-lg border border-separator/60 bg-pane/30 overflow-hidden">
      <button
        onClick={onToggle}
      ...
      {open && (
        <div className="border-t border-separator/50 p-3">{children}</div>
      )}
    </section>
  );
}
...
function LabeledInput({
  ...
  return (
    <div>
      <label className="mb-1 block text-[10px] text-text-primary">
        {label}
      </label>
  ...
  return (
    <div>
      <div className="mb-2 flex items-center justify-between gap-3">
        <label className="block text-[10px] text-text-primary">{label}</label>
        <button
          onClick={onImport}
          disabled={disabled}
          className={`
            inline-flex items-center gap-1 rounded-md px-2 py-1 text-[11px] cursor-default
            ${disabled ? "bg-hover text-text-secondary/60" : "bg-hover text-text-primary hover:bg-hover/80"}
          `}
        >
      ...
      {values.length === 0 ? (
        <div className="rounded-md border border-dashed border-separator/60 px-3 py-2 text-[11px] text-text-secondary/70">
          {emptyLabel}
        </div>
      ) : (
        <div className="space-y-2">
          {values.map((value, index) => (
            <div
              key={`${value}-${index}`}
              className="flex items-center gap-2 rounded-md border border-separator/50 bg-canvas px-2.5 py-2"
            >
...
function CompactSingleFilePickerField({
  ...
  return (
    <div>
      <div className="mb-1 block text-[10px] text-text-primary">{label}</div>
      <div className="flex h-9 items-center gap-2">
        <div className="min-w-0 flex-1">
          {hasValue ? (
            <div className="flex h-9 items-center gap-2 rounded-md border border-separator/50 bg-canvas px-2.5">
              <span className="min-w-0 flex-1 truncate text-[12px] text-text-primary">
                {value}
              </span>
              <button
            ...
            <div className="flex h-9 items-center rounded-md border border-dashed border-separator/60 px-3 text-[11px] text-text-secondary/70">
              {emptyLabel}
            </div>
          )}
        </div>
        ...
        <button
          ...
          disabled={disabled}
          className={`
            inline-flex h-9 shrink-0 items-center gap-1 rounded-md px-3 text-[11px] cursor-default
            ${disabled ? "bg-hover text-text-secondary/60" : "bg-hover text-text-primary hover:bg-hover/80"}
          `}
        >
          <MdFolderOpen size={14} />
