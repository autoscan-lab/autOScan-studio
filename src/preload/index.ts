...
export const api = {
  // Workspace
  loadWorkspace: (rootPath: string) => ipcRenderer.invoke('workspace:load', rootPath),
  readFile: (filePath: string) => ipcRenderer.invoke('workspace:read-file', filePath),
  listPolicies: (rootPath: string) => ipcRenderer.invoke('workspace:list-policies', rootPath),
  readPolicy: (policyPath: string) => ipcRenderer.invoke('workspace:read-policy', policyPath),
  ...
  deletePolicy: (policyPath: string) => ipcRenderer.invoke('workspace:delete-policy', policyPath),
  fileExtension: (filePath: string) => ipcRenderer.invoke('workspace:file-extension', filePath),
  importFiles: (rootPath: string, subdir: string, filePaths: string[]) =>
    ipcRenderer.invoke('workspace:import-files', rootPath, subdir, filePaths),

  // Dialogs
  openDirectory: () => ipcRenderer.invoke('dialog:open-directory'),
  openFiles: (title: string, filters?: { name: string; extensions: string[] }[]) =>
    ipcRenderer.invoke('dialog:open-files', title, filters),

  // Engine
  runSession: (workspacePath: string, policyPath: string, submissionPath?: string) =>
    ipcRenderer.invoke('engine:run-session', workspacePath, policyPath, submissionPath),
